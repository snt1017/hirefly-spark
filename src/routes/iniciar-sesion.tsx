...
export const Route = createFileRoute("/iniciar-sesion")({
  head: () => ({ meta: [{ title: "Iniciar sesión — Hirefly" }, { name: "description", content: "Ingresa a Hirefly." }] }),
  component: Login,
});
      ...
      <div className="mx-auto max-w-md px-6">
        ...
        <div className="mt-6 rounded-xl border border-border bg-card p-8 shadow-sm">
          ...
          <div className="mt-4 rounded-md bg-primary-light p-3 text-xs text-primary">
            Cuenta demo: <span className="font-medium">demo@hirefly.co</span> · <span className="font-medium">demo1234</span>
          </div>
          <form onSubmit={form.handleSubmit(onSubmit)} className="mt-6 space-y-4">
            ...
            <div className="flex items-center gap-2">
              <Checkbox id="remember" checked={form.watch("remember")} onCheckedChange={(v) => form.setValue("remember", v === true)} />
          ...
          <p className="mt-4 text-center text-sm text-muted-foreground">
            ¿No tienes cuenta? <Link to="/registro" className="font-medium text-primary hover:underline">Crea una gratis</Link>
          </p>
        </div>
      </div>
    </div>
